# -*- coding: utf-8 -*-
"""
Created on Tue Jul  4 20:31:58 2023
...
def menu():
    image = os.path.join(os.path.dirname(__file__), "ml.png")

    for widget in fenetre.winfo_children():
        widget.destroy()

    texte = Label(fenetre, text="\nWelcome to the EFREIMOTION app\n")
    texte.pack()

    image = Image.open(image)
    image = image.resize((200, 200), Image.ANTIALIAS)  # Redimensionner l'image
    image_tk = ImageTk.PhotoImage(image)

    label_image = Label(fenetre, image=image_tk)
    label_image.pack()
...
def button_text():
    ...
    bouton_analyse = tk.Button(fenetre, text="Analyze the text", command = recuperer_texte)
    bouton_analyse.pack()
